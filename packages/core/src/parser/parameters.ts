 ...
 * The `:use` parameter controls block behavior through the Render API.
 */
 ...
 *
 * @example
 * parseCodeBlockParameters(":use dom | withSourceCode")
 * // Returns: {
...
/**
 * Check if block is an API endpoint
 *
 * API blocks define server-side endpoints and should not be hydrated in the browser.
 *
 * @param params - Block parameters
 * @returns True if :use api
 */
export function isApiBlock(params: BlockParameters): boolean {
  const useValue = params.use || "";
  const mode = useValue.split("|")[0].trim();
  return mode === "api";
}
