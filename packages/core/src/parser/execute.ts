...
function transformToExportLastExpression(code: string): string {
  ...
  if (
    /^(const|let|var|function|class|async\s+function|interface|type|enum)\s/.test(
      lastLine,
    )
  ) {
  ...
  if (
    /^(if|else|for|while|do|switch|try|catch|finally|throw)\b/.test(lastLine)
  ) {
    return `const __result = await (async () => { ${code} })();\nexport default __result;`;
  }
 ...
 */
async function transpileCode(code: string, language: string): Promise<string> {
