...
import { processOrgElements } from "./exporter.ts";
...
export async function parseOrgContent(
  ...
  // Strip shebang line if present (for hashbang mode)
  ...
  // 3. Process all org elements (code blocks and drawers) in single AST walk
  const { virtualModules, cacheFiles, collectedBlocks, modifiedAst } = await processOrgElements(
    ast,
    context,
    contentHelpers
  );
