...
async function processClientBlock(
  ...
  // - Other languages (html, json, etc.): use plugin's defaultExtension (usually 'js')
  ...
  const cacheExtension = keepOriginalExtension ? block.language : (plugin.defaultExtension || block.language);
  ...
  // Only register blocks that have a default export (render function)
  // Library blocks with only named exports shouldn't be hydrated
  if (context.hydrateRegistry) {
    const hasDefaultExport = code.includes("export default") || code.includes("export{default");
    if (hasDefaultExport) {
      const ext = plugin.defaultExtension || block.language;
      context.hydrateRegistry.addModule(context.orgFilePath, blockId, ext, cachePath);
    }
  }

  // Try to extract static HTML for pre-rendering (SSG)
  // This allows content to be visible before JavaScript loads
  const staticHtml = extractStaticHtml(code);

  // Generate container div with data attribute for hydration
  // Include static HTML if available for pre-rendering
  const containerDiv = staticHtml
    ? `<div id="${containerId}" data-org-block="${blockId}" class="org-block-result">${staticHtml}</div>`
    : `<div id="${containerId}" data-org-block="${blockId}" class="org-block-result"></div>`;
