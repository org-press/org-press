...
async function processClientBlock(
  ...
  // Only register blocks that need hydration:
  // - Have a default export (dom mode), OR
  // - Have a render export (react mode)
  // Library blocks with only other named exports shouldn't be hydrated
  if (context.hydrateRegistry) {
    ...
    if (hasDefaultExport || hasRenderExport) {
      const ext = plugin.defaultExtension || block.language;
      context.hydrateRegistry.addModule(context.orgFilePath, blockId, ext, cachePath, moduleId, modeName);
    }
  }
