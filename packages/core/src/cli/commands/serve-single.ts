...
import type { OrgPressConfig as PluginConfig, OrgPressPlugin } from "../../plugins/types.ts";
import { PluginRegistry } from "../../plugins/registry.ts";
import { defaultPlugins } from "../../plugins/presets.ts";
...
export async function serveSingleFile(
  ...
  // Get plugins from config (for single file mode, this is typically empty)
  // Users must include defaultPlugins if they want built-in functionality
  ...
  // Store plugin context in config for pipeline access
  (config as any)._pluginContext = ctx;
  ...
  const server = await createServer({
    ...
    optimizeDeps: {
      // Disable dependency scanning entirely for single-file mode
      // This prevents Vite from scanning the entire directory tree
      noDiscovery: true,
      include: [],
      entries: [],
    },
