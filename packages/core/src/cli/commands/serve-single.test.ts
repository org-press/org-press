...
describe("Serve Single File", () => {
  ...
  describe("middleware registration", () => {
    ...
    it("should include optimizeDeps for React JSX runtime", async () => {
      const { serveSingleFile } = await import("./serve-single.ts");
      ...
      try {
        ...
        // Verify optimizeDeps includes React JSX runtime
        expect(createServerCall.optimizeDeps).toBeDefined();
        expect(createServerCall.optimizeDeps.include).toContain("react");
        expect(createServerCall.optimizeDeps.include).toContain("react/jsx-runtime");
        expect(createServerCall.optimizeDeps.include).toContain("react/jsx-dev-runtime");
      } finally {
        // Cleanup
        if (fs.existsSync(tempFile)) fs.unlinkSync(tempFile);
        if (fs.existsSync(tempDir)) fs.rmdirSync(tempDir);
