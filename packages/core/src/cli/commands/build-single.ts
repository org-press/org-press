 ...
 * wraps the code to capture the last expression's result.
 */
function transformToExportLastExpression(code: string): string {
  ...
  // Transform the last expression to an export
...
async function executeServerBlock(code: string, language: string): Promise<string> {
  ...
  try {
    // Transform to export the last expression if no explicit export
    const transformedCode = transformToExportLastExpression(code);
...
/**
 * Executable languages for preview blocks
 */
const PREVIEW_EXECUTABLE_LANGUAGES = ["javascript", "js", "typescript", "ts", "tsx", "jsx"];
...
async function renderOrgToHtml(
  ...
  // Collect server and preview blocks for async execution
  const serverBlocks: Array<{ id: string; code: string; language: string; isPreview?: boolean }> = [];
  let blockCounter = 0;
  ...
  function renderNode(node: any): string {
    ...
    switch (node.type) {
      ...
      case "src-block": {
        ...
        // Handle dom blocks - execute if the language is supported
        if (useFirst === "dom" && PREVIEW_EXECUTABLE_LANGUAGES.includes(lang.toLowerCase())) {
          const blockId = `__PREVIEW_BLOCK_${blockCounter++}__`;
          serverBlocks.push({ id: blockId, code, language: lang, isPreview: true });

          if (hasSourceCode) {
            // Show both source code and output
            return `<div class="org-source-code">
              <div class="org-source-label">Source</div>
              ${codeHtml}
            </div>
            <div class="org-preview-output">${blockId}</div>`;
          }
          // Preview only - show output
          return `<div class="org-preview-output">${blockId}</div>`;
        }

        // For dom blocks with withSourceCode but non-executable language, add a source code label
        if ((useFirst === "dom" || useFirst === "") && hasSourceCode) {
          return `<div class="org-source-code">
            <div class="org-source-label">Source</div>
            ${codeHtml}
          </div>`;
  ...
  // Execute server and preview blocks and replace placeholders
  if (serverBlocks.length > 0) {
    const serverCount = serverBlocks.filter(b => !b.isPreview).length;
    const previewCount = serverBlocks.filter(b => b.isPreview).length;
    if (serverCount > 0) {
      console.log(`[build:static] Executing ${serverCount} server block(s)...`);
    }
    if (previewCount > 0) {
      console.log(`[build:static] Executing ${previewCount} preview block(s)...`);
    }
    ...
    const results = await Promise.all(
      serverBlocks.map(async (block) => {
        try {
          ...
          return { id: block.id, output, isPreview: block.isPreview };
        } catch (error) {
          const errorMsg = error instanceof Error ? error.message : String(error);
          return { id: block.id, output: `<span class="org-error">Error: ${escapeHtml(errorMsg)}</span>`, isPreview: block.isPreview };
        }
      })
    );
