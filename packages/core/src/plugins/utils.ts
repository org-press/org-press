 ...
 *   #+begin_src javascript :use dom | withSourceCode :height 400px
 *
 ...
 *
 * @example
 * parseBlockParameters(":use dom | withSourceCode :height 400px")
...
export function parseBlockParameters(
  ...
  // Special handling for :use parameter - can contain pipe syntax with spaces
  // Pattern: :use value | wrapper1 | wrapper2 (capture until next :key or end)
  const useMatch = meta.match(/:use\s+((?:[^:]|:(?!\w))+?)(?=\s*:\w|$)/);
  if (useMatch) {
    params.use = useMatch[1].trim();
  }

  // Match :key value patterns for other parameters
  ...
  while ((match = keyValueRegex.exec(meta)) !== null) {
    ...
    // Skip :use - already handled specially
    if (key === "use") continue;
    params[key] = value;
  }
