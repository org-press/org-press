 ...
 * Pipe Parser
 *
 * Parses the `:use` parameter pipe syntax for composable Render API.
 *
 * Syntax: `:use mode | wrapper1 | wrapper2?config=value`
 *
 * Examples:
 * - `:use dom` - just dom mode
 * - `:use dom | withTabs` - dom mode with tabs wrapper
 * - `:use dom | withSourceCode?position=before` - with query string config
 * - `:use dom | withContainer:{"class":"highlight"}` - with JSON config
