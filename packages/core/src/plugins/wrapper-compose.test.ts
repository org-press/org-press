...
import type { RenderFunction, Wrapper, WrapperFactory, BlockContext } from "./preview.ts";
...
describe("Wrapper Composition", () => {
  ...
  describe("composeWrappers", () => {
    it("should return base preview for empty segments", async () => {
      const baseRender: RenderFunction = () => "base";
      const result = await composeWrappers([], { baseRender });
    ...
    it("should return base preview for single segment (mode only)", async () => {
      ...
      const baseRender: RenderFunction = () => "base";
      const result = await composeWrappers(segments, { baseRender });
    ...
    it("should apply single wrapper", async () => {
      ...
      const baseRender: RenderFunction = (result) => `${result}`;
      const composed = await composeWrappers(segments, { registry, baseRender });
  ...
  describe("composeWrappersSync", () => {
    it("should compose wrappers synchronously", () => {
      ...
      const baseRender: RenderFunction = (result) => `${result}`;
      const composed = composeWrappersSync(segments, { registry, baseRender });
  ...
  describe("identityWrapper", () => {
    it("should return the render unchanged", () => {
      const wrapper = identityWrapper();
      const renderFn: RenderFunction = () => "original";
      const wrapped = wrapper(renderFn);
  ...
  describe("createSimpleWrapper", () => {
    it("should create a wrapper from a transform function", () => {
      const factory = createSimpleWrapper((result, render) => {
        return `transformed:${result}`;
      });

      const wrapper = factory();
      const renderFn: RenderFunction = (result) => `render:${result}`;
      const wrapped = wrapper(renderFn);
