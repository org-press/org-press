...
describe("Plugin Utilities", () => {
  ...
  describe("parseBlockParameters", () => {
    it("should parse simple parameters", () => {
      const params = parseBlockParameters(":use dom :height 400px");
    ...
    it("should treat parameters without values as flags", () => {
      const params = parseBlockParameters(":exec :use dom");
      expect(params).toEqual({
        exec: "",
        use: "dom",
      });
    ...
    it("should parse pipe syntax in :use parameter", () => {
      const params = parseBlockParameters(":use dom | withSourceCode");
      expect(params).toEqual({
        use: "dom | withSourceCode",
      });
    });

    it("should parse pipe syntax with multiple wrappers", () => {
      const params = parseBlockParameters(":use server | json | cache");
      expect(params).toEqual({
        use: "server | json | cache",
      });
    });

    it("should parse pipe syntax with additional parameters", () => {
      const params = parseBlockParameters(":use dom | withSourceCode :height 400px :name example");
      expect(params).toEqual({
        use: "dom | withSourceCode",
        height: "400px",
        name: "example",
    ...
    it("should handle pipe syntax without spaces", () => {
      const params = parseBlockParameters(":use dom|withSourceCode");
      expect(params).toEqual({
        use: "dom|withSourceCode",
      });
