...
describe("Pipe Parser", () => {
  ...
  describe("getMode", () => {
    it("should return first segment name", () => {
      expect(getMode("dom | withTabs")).toBe("dom");
    });
    ...
    it("should default to dom for empty", () => {
      expect(getMode("")).toBe("dom");
    });
  });
...
describe("resolveUse", () => {
  ...
  it("should use defaultUse when no useParam and no language default", () => {
    const result = resolveUse({
      language: "typescript",
      defaultUse: "dom | withSourceCode",
    });
    expect(result).toBe("dom | withSourceCode");
  });

  it("should fall back to 'dom' when no config provided", () => {
    const result = resolveUse({
      language: "typescript",
    });
