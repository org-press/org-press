/**
 * Tests for built-in render wrappers
 */
...
const simpleRender: RenderFunction = (result) => {
  return `<div class="result">${String(result)}</div>`;
};

// Throwing render function for error boundary testing
const throwingRender: RenderFunction = () => {
  throw new Error("Test error");
};

describe("withSourceCode", () => {
  it("shows source code after render output by default", () => {
    const wrapper = withSourceCode();
    const wrapped = wrapper(simpleRender);
  ...
  it("shows source code before render output", () => {
    ...
    expect(result).toContain('<div class="result">42</div>');
    expect(result).toContain('<div class="org-source-code">');
    // Source should come first (before render output)
...
describe("withContainer", () => {
  it("wraps render output in container with default class", () => {
  ...
  it("returns null for null render result", () => {
    const nullRender: RenderFunction = () => null;
    const wrapper = withContainer();
    const wrapped = wrapper(nullRender);
    const ctx = createMockContext();
...
describe("withErrorBoundary", () => {
  it("passes through successful render", () => {
  ...
  it("catches errors and shows fallback", () => {
    const wrapper = withErrorBoundary();
    const wrapped = wrapper(throwingRender);
    const ctx = createMockContext();
    ...
    expect(result).toContain("Error rendering block");
...
describe("withConsole", () => {
  ...
  it("displays console output after render output by default", () => {
    const wrapper = withConsole();
    const wrapped = wrapper(simpleRender);
    const ctx = createMockContext();
    ...
    // Render output should come first
    expect((result as string).indexOf('<div class="result">')).toBeLessThan(
      (result as string).indexOf('<div class="org-console">')
    );
  });

  it("displays console output before render output", () => {
...
describe("withCollapse", () => {
  it("wraps render output in collapsible details element", () => {
