 ...
 *
 * Catches errors during preview rendering and displays a fallback.
 */
 ...
 * ":use preview | withErrorBoundary"
 *
 * // With custom fallback
 * ":use preview | withErrorBoundary?fallback=Something went wrong"
 *
 * // Show error details
 * ":use preview | withErrorBoundary?showError&showStack"
 * ```
 */
export const withErrorBoundary: WrapperFactory = (config?: Record<string, unknown>) => {
  const {
    fallback = "Error rendering preview",
