...
export interface WithConsoleConfig {
  /** Where to show console output: before, after, or replace render output */
 ...
 * ":use dom | withConsole?position=before"
 *
 * // Limit output
 * ":use dom | withConsole?maxLines=10"
 * ```
 */
export const withConsole: WrapperFactory = (config?: Record<string, unknown>) => {
  ...
  return (render: RenderFunction): RenderFunction => {
    return (result: unknown, ctx: BlockContext): RenderResult => {
      ...
      // Get render output of actual result
      const renderResult = render(actualResult, ctx);
