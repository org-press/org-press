 ...
 *   ./script.org dev              Start dev server for this file
 *   ./script.org serve            Start dev server for this file (alias)
...
import type { OrgPressPlugin, CommandContext, ParsedArgs as PluginParsedArgs, ArgDefinition } from "../plugins/types.ts";
import { parseArgs as parsePluginArgs } from "../plugins/factories/create-command.ts";
...
COMMANDS:
  ...
  try {
    ...
    const { initPluginContext, getPluginContext } = await import("../plugins/plugin-init.ts");
    await initPluginContext(config as unknown as CommandContext["config"], projectRoot);
...
PLUGIN COMMANDS:`);
      for (const [name, options] of commands) {
        ...
        console.log(`  orgp ${name.padEnd(16)} ${desc}`);
      }
    }
  } catch {
    // If config loading fails, show built-in CLI commands
    console.log(`
PLUGIN COMMANDS:
...
async function runOrgFile(args: CLIParsedArgs): Promise<void> {
...
async function tryPluginCommand(command: string, args: string[]): Promise<number | null> {
  ...
  // Initialize plugin context singleton to register all command handlers
  const { initPluginContext, getPluginContext } = await import("../plugins/plugin-init.ts");
  await initPluginContext(config as unknown as CommandContext["config"], projectRoot);
  const ctx = getPluginContext()!;

  // Get command from plugin context
  const commandHandler = ctx.getCommand(command);
  if (commandHandler) {
    const parsedArgs = parsePluginArgs(args, commandHandler.args || []);
    const context: CommandContext = {
      config: config as unknown as CommandContext["config"],
      projectRoot,
      contentDir,
    };

    return commandHandler.execute(parsedArgs, context);
  }
...
async function handleHashbangInvocation(args: string[]): Promise<void> {
  ...
  switch (invocation.subcommand) {
    ...
    case "dev":
    case "serve": {
      // Serve single file (dev is alias for serve)
      // Port priority: --port flag > ORGP_PORT env > PORT env > default 3000
      const portIndex = invocation.args.indexOf("--port");
      let port = 3000;
      if (portIndex !== -1 && invocation.args[portIndex + 1]) {
        port = parseInt(invocation.args[portIndex + 1], 10);
      } else if (process.env.ORGP_PORT) {
        port = parseInt(process.env.ORGP_PORT, 10);
      } else if (process.env.PORT) {
        port = parseInt(process.env.PORT, 10);
      }

      // Host: --host flag or default to localhost
      // Use "0.0.0.0" to expose to network (useful for containers/remote access)
      const hostIndex = invocation.args.indexOf("--host");
      let host = "localhost";
      if (hostIndex !== -1) {
        const nextArg = invocation.args[hostIndex + 1];
        // --host without value means 0.0.0.0 (expose to all interfaces)
        host = (nextArg && !nextArg.startsWith("-")) ? nextArg : "0.0.0.0";
      }
...
Subcommands:
  ...
  dev             Start dev server for this file
  serve           Start dev server (alias for dev)
...
Options (dev/serve):
  ...
  --host [addr]   Host to bind to (default: localhost, --host alone = 0.0.0.0)
