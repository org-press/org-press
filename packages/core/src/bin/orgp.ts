...
COMMANDS:
  ...
  try {
    ...
    const registry = new PluginRegistry(config as unknown as CommandContext["config"], projectRoot);
    await registry.register(configPlugins as OrgPressPlugin[]);
...
async function tryPluginCommand(command: string, args: string[]): Promise<number | null> {
  ...
  // Initialize plugin registry to register all command handlers
  const { PluginRegistry } = await import("../plugins/registry.ts");
  const registry = new PluginRegistry(config as unknown as CommandContext["config"], projectRoot);
  await registry.register(configPlugins as OrgPressPlugin[]);
  const ctx = registry.getContext();
