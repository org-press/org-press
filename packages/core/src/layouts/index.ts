 ...
 * 1. **Built-in themes** (e.g., "org-press:minimal", "org-press:default"):
 *    - Loads built-in layouts from org-press package
 * 2. **File path** (e.g., ".org-press/themes/index.tsx"):
 *    - Default export (Layout component)
 *    - Named export `layouts` object
 *    - Named export `getLayout(name)` function
 * 3. **Directory path** (e.g., ".org-press/theme"):
 ...
 * 4. **Fallback**: org-press default layout
 *
 * @param themePath - Path to theme file or directory, or built-in theme name
 * @param layoutName - Requested layout name (default: "default")
 * @returns Layout component
 *
 * @example
 * // Built-in minimal theme (for single-file mode)
 * const Layout = await loadLayout("org-press:minimal");
 *
 * @example
 * // File path
...
export async function loadLayout(
  ...
  // Handle built-in org-press themes (e.g., "org-press:minimal")
  if (themePath.startsWith("org-press:")) {
    const builtinTheme = themePath.slice("org-press:".length);
    if (builtinTheme === "minimal") {
      return loadMinimalLayout();
    }
    // Default to standard layout for unknown built-in themes
...
async function loadLayoutFromFile(
  ...
  // Check if theme file exists before attempting import
  // This avoids confusing error messages when no custom theme is configured
  if (!existsSync(absolutePath)) {
    // Silently fall back to default layout - no custom theme is normal
    return loadDefaultLayout();
  }

  try {
    const themeModule = await import(/* @vite-ignore */ absolutePath);
...
async function loadLayoutFromDirectory(
  ...
  try {
    const layoutModule = await import(/* @vite-ignore */ join(basePath, "Layout.tsx"));
  ...
  try {
    const layoutsModule = await import(/* @vite-ignore */ join(basePath, "layouts.tsx"));
  ...
  try {
    const indexModule = await import(/* @vite-ignore */ join(basePath, "index.tsx"));
...
export async function loadDefaultLayout(): Promise<LayoutComponent> {
  ...
  try {
    const defaultModule = await import(/* @vite-ignore */ "./default/Layout.tsx");
...
/**
 * Load minimal layout from org-press
 *
 * A clean layout without header, footer, or navigation.
 * Ideal for single-file hashbang mode and standalone pages.
 *
 * @returns Minimal layout component
 */
export async function loadMinimalLayout(): Promise<LayoutComponent> {
  const cacheKey = "org-press:minimal";
  ...
  try {
    const minimalModule = await import(/* @vite-ignore */ "./minimal/Layout.tsx");
    const layout = minimalModule.default as LayoutComponent;
    layoutCache.set(cacheKey, layout);
    return layout;
  } catch (error) {
    throw new Error(
      `Failed to load minimal layout: ${error instanceof Error ? error.message : String(error)}`,
    );
  }
}
...
export async function preloadLayouts(
  ...
  await Promise.all(layoutNames.map((name) => loadLayout(themePath, name)));
}
