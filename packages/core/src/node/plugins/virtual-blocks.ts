...
import type { CodeBlock } from "../../plugins/types.ts";
import {
  parseBlockParameters,
  createVirtualModuleId as createVMID,
...
function hasRenderExport(code: string): boolean {
  return (
    /export\s+(?:const|function|{[^}]*render[^}]*})/.test(code) &&
    /\brender\b/.test(code)
  );
}
...
/**
 * Find a handler for a code block
 *
 * Matching priority:
      ...
      const loader =
        language.toLowerCase() === "tsx"
          ...
          : language.toLowerCase() === "jsx"
            ? "jsx"
            : "ts";
      transpiledCode = await transformTypeScript(
        code,
        loader as "ts" | "tsx" | "jsx",
      );
    }
...
function parseVirtualModuleId(id: string): VirtualModuleId | null {
  ...
  let match = cleanId.match(
    /^virtual:org-press:block:([^:]+):(.+):(\d+)\.(\w+)$/,
  );
  if (match) {
  ...
  match = cleanId.match(
    /^virtual:org-press:block:([^:]+):(.+):NAME:([^.:]+)\.(\w+)$/,
  );
  if (match) {
...
function generateOrgImportModule(
  ...
  blocks.forEach((block, idx) => {
    ...
    if (block.name) {
      const jsIdentifier = block.name
        .replace(/-/g, "_")
        .replace(/[^a-zA-Z0-9_]/g, "");
      namedBlocksEntries.push(
        `${JSON.stringify(block.name)}: __blocks[${idx}]`,
      );
 ...
 *
 * @param plugins Array of plugins (for context compatibility)
  ...
  return {
    ...
    resolveId(id: string, importer: string | undefined) {
      ...
      if (id === "org-press/client/hydrate-runtime") {
        ...
        const distClientPath = path.resolve(
          currentDir,
          "client/hydrate-runtime.ts",
        );
        const srcClientPath = path.resolve(
          currentDir,
          "../src/client/hydrate-runtime.ts",
        );
      ...
      if (isOrgImport(id) && id.includes("?")) {
        ...
        const resolvedOrgPath = path.resolve(
          process.cwd(),
      ...
      if (
        ...
        if (!resolvedRelativePath) {
          console.warn(
            `[virtual-blocks] Cannot resolve relative path without importer: ${id}`,
          );
          return null;
        }
    ...
    async load(id: string) {
      ...
      if (id.startsWith("\0org-import:")) {
        ...
        const code = generateOrgImportModule(fullPath, orgFilePath, options);
      ...
      // Get plugin context from config
      const pluginContext = options.config?._pluginContext as
        | PluginContextImpl
        | undefined;
      ...
      try {
        ...
        if (!pluginContext) {
          this.error(
            `No plugin context available. Plugin "${pluginName}" requires setup.`,
          );
          return null;
        }

        // Try block handler first
        const blockHandler = pluginContext.getBlockHandler(pluginName);
        if (blockHandler) {
          const result = await blockHandler(codeBlock, blockCtx);
          code = result.code || "";
        } else {
          // Try transformer handler
          const transformer = pluginContext.getTransformer(pluginName);
          if (transformer && transformer.onBuild) {
            ...
            this.error(
              `Make sure the plugin is registered with CreateBlock or CreateTransformer.`,
            );
            return null;
          }
        }
    ...
    async handleHotUpdate({ file, server, timestamp }) {
      if (file.endsWith(".org")) {
        ...
        const { clearCrossFileCache } =
          await import("../../render/cross-file-layout.ts");
