...
import type { OrgPressPlugin, MiddlewareOptions } from "../plugins/types.ts";
...
import { initPluginContext, getPluginContext } from "../plugins/plugin-init.ts";
...
export async function orgPress(
  ...
  // Initialize plugin context singleton (creates PluginRegistry, calls setup() on each plugin)
  await initPluginContext(config, process.cwd());

  // Initialize render API (registers built-in wrappers like withTabs, withSourceCode)
  initializeRenderApi();

  return createOrgPressPlugins(config, plugins as any);
}
