...
import type { OrgPressConfig, LayoutComponent, OrgPressPlugin } from "org-press";
import {
  getContentPages,
  getOrgFileFromUrl,
  loadConfig,
...
let cachedPlugins: OrgPressPlugin[] | null = null;
...
async function initialize(): Promise<void> {
  ...
  // Initialize plugin context singleton so getPluginContext() works during rendering
  await initPluginContext(cachedConfig);

  // Initialize render API (registers built-in wrappers like withTabs, withSourceCode)
  initializeRenderApi();

  // Get plugins from config
  cachedPlugins = cachedConfig.plugins;
