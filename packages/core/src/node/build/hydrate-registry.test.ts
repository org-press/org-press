...
describe("HydrateRegistry", () => {
  ...
  describe("generateHydrateEntry", () => {
    ...
    it("should fall back to cache path when virtualModuleId is not provided and warn", () => {
      ...
      // Should warn about missing virtualModuleId
      expect(warnSpy).toHaveBeenCalledWith(
        expect.stringContaining('Block "test-block" has no virtualModuleId')
      );
      warnSpy.mockRestore();
    });
  });
  ...
  describe("addModule", () => {
    ...
    it("should throw on empty orgPath", () => {
      expect(() =>
        registry.addModule("", "block-0", "tsx", "/cache/block.tsx", "virtual:test", "dom")
      ).toThrow("[HydrateRegistry] orgPath is required");
    });

    it("should throw on empty blockId", () => {
      expect(() =>
        registry.addModule("content/test.org", "", "tsx", "/cache/block.tsx", "virtual:test", "dom")
      ).toThrow("[HydrateRegistry] blockId is required");
    });

    it("should throw on empty ext", () => {
      expect(() =>
        registry.addModule("content/test.org", "block-0", "", "/cache/block.tsx", "virtual:test", "dom")
      ).toThrow("[HydrateRegistry] ext is required");
    });

    it("should warn but not throw on empty virtualModuleId", () => {
      const warnSpy = vi.spyOn(console, "warn").mockImplementation(() => {});
      expect(() =>
        registry.addModule("content/test.org", "block-0", "tsx", "/cache/block.tsx", "", "dom")
      ).not.toThrow();
      expect(warnSpy).toHaveBeenCalledWith(
        expect.stringContaining("Missing virtualModuleId")
      );
      warnSpy.mockRestore();
    });
  });
