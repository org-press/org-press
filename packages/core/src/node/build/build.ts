...
import { initPluginContext } from "../../plugins/plugin-init.ts";
...
export async function build(options: BuildOptions): Promise<BuildResult> {
  ...
  // Initialize plugin context so getPluginContext() is available during pre-parse
  await initPluginContext(config);
...
async function buildSSRBundle(config: OrgPressConfig): Promise<void> {
  ...
  await viteBuild({
    ...
    ssr: {
      // Externalize all node_modules - avoids bundling build tools
      // (vite, rollup, esbuild) that use CJS require() which conflicts
      // with ESM top-level await in Node 25+
      external: true as unknown as string[],
    },
    resolve: {
      alias: {
        "#root": process.cwd(),
...
async function prerenderPages(
  ...
  // Vite outputs .mjs for ESM projects by default, fall back to .js
  const serverEntryMjs = path.resolve(
    config.cacheDir,
    "server/entry-generate.mjs"
  );
  const serverEntryJs = path.resolve(
    config.cacheDir,
    "server/entry-generate.js"
  );
  const serverEntryPath = fs.existsSync(serverEntryMjs)
    ? serverEntryMjs
    : serverEntryJs;

  if (!fs.existsSync(serverEntryPath)) {
    throw new Error(
      `Server entry not found at ${serverEntryJs} or ${serverEntryMjs}. Run server build first.`
    );
  }
...
/**
 * Collect all CSS files from a manifest entry and its imports (recursively)
 */
function collectCssFromManifest(
  ...
  // Find the manifest entry by file path
  let entry: ViteManifest[string] | undefined;
  for (const [key, value] of Object.entries(viteManifest)) {
    if (value.file === entryFile || `/${value.file}` === entryFile) {
      entry = value;
      break;
    }
  }
  ...
  if (entry.imports) {
    for (const importKey of entry.imports) {
      ...
      if (importEntry) {
        ...
        // Recurse into imports of imports
        if (importEntry.imports) {
          collectCssFromManifest(viteManifest, importEntry.file, collected);
        }
      }
    }
  }
 ...
 *
 * This function also collects and injects CSS from the hydrate entry's
 * dependencies, since Vite only auto-links CSS for the main entry points.
 *
...
function injectHydration(
  ...
  // Collect CSS from the hydrate entry and its imports
  const cssFiles = collectCssFromManifest(
    assetManifest.viteManifest,
    hydrateScriptPath.replace(/^\//, "") // Remove leading slash for manifest lookup
  );

  // Apply base path if needed
  const normalizedBase = (config.base && config.base !== "/")
    ? config.base.replace(/\/$/, "")
    : "";

  // Generate CSS link tags
  const cssLinks = cssFiles.map(css => {
    const cssPath = normalizedBase ? `${normalizedBase}/${css}` : `/${css}`;
    return `<link rel="stylesheet" href="${cssPath}"/>`;
  }).join("");

  // Generate the script tag
  let scriptSrc = hydrateScriptPath;
  if (normalizedBase && !scriptSrc.startsWith(normalizedBase)) {
    scriptSrc = `${normalizedBase}${scriptSrc}`;
  }
  const hydrateScript = `<script type="module" src="${scriptSrc}"></script>`;

  // Inject CSS into head and script before </body>
  let result = html;
  if (cssLinks && result.includes("</head>")) {
    result = result.replace("</head>", `${cssLinks}</head>`);
  }
  if (result.includes("</body>")) {
    result = result.replace("</body>", `${hydrateScript}</body>`);
  } else {
    result = result + hydrateScript;
  }
