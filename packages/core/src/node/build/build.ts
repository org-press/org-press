    ...
    build: {
      ...
      rollupOptions: {
        external: (id) => {
          ...
          // Externalize build tools that use CommonJS internals (__dirname, etc.)
          // These should never be bundled into the SSR output
          const buildTools = [
            "rollup",
            "@rollup/",
            "vite",
          ...
          for (const tool of buildTools) {
            if (id === tool || id.startsWith(tool + "/") || id.startsWith(tool)) {
              return true;
            }
          }
