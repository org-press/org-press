...
export function generateHydrateEntry(orgPath: string, blocks: BlockEntry[], cacheDir?: string): string {
  ...
  // Render a value to the element
  lines.push("function render(el, value) {");
  lines.push("  // Skip null/undefined");
  ...
  lines.push("    value(el.id);");
  lines.push("  }");
  lines.push("  // Handle HTMLElement exports");
  lines.push("  else if (value instanceof HTMLElement) {");
  lines.push("    if (!el.hasChildNodes()) el.appendChild(value);");
  lines.push("  }");
  lines.push("  // Handle string exports (could be HTML or plain text)");
  lines.push("  else if (typeof value === \"string\") {");
  lines.push("    if (value.trim().startsWith(\"<\")) {");
  lines.push("      el.innerHTML = value;");
  lines.push("    } else {");
  lines.push("      el.textContent = value;");
  lines.push("    }");
  lines.push("  }");
  lines.push("  // Handle numbers and booleans");
  lines.push("  else if (typeof value === \"number\" || typeof value === \"boolean\") {");
  lines.push("    el.textContent = String(value);");
  lines.push("  }");
  lines.push("  // Handle objects/arrays (display as formatted JSON)");
  lines.push("  else if (typeof value === \"object\") {");
  lines.push("    el.innerHTML = \"<pre>\" + JSON.stringify(value, null, 2) + \"</pre>\";");
  lines.push("  }");
  lines.push("}");
  lines.push("");

  // Hydrate function - handles different export types
  lines.push("async function hydrate() {");
  ...
  lines.push("    // Get the value - handle both direct exports and default exports");
  lines.push("    let value = mod && typeof mod === \"object\" && \"default\" in mod ? mod.default : mod;");
  lines.push("    // Await if value is a Promise (from async IIFE wrapper)");
  lines.push("    if (value && typeof value.then === \"function\") {");
  lines.push("      value = await value;");
  lines.push("    }");
  lines.push("    render(el, value);");
  lines.push("  }");
  lines.push("}");
