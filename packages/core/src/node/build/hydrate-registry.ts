...
export function generateHydrateEntry(orgPath: string, blocks: BlockEntry[], cacheDir?: string): string {
  ...
  blocks.forEach((block, index) => {
    if (!block.virtualModuleId) {
      console.warn(
        `[HydrateRegistry] Block "${block.blockId}" has no virtualModuleId, using cachePath as fallback`
      );
    }
...
export class HydrateRegistry {
  ...
  addModule(
    ...
    if (!orgPath) throw new Error("[HydrateRegistry] orgPath is required");
    if (!blockId) throw new Error("[HydrateRegistry] blockId is required");
    if (!ext) throw new Error("[HydrateRegistry] ext is required");
    if (!virtualModuleId) {
      console.warn(
        `[HydrateRegistry] Missing virtualModuleId for block "${blockId}" in ${orgPath}, falling back to cachePath`
      );
    }
