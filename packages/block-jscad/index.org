...
/**
 * Org-Press JSCad Block Plugin (v2)
 *
 ...
 */
export const jscadPlugin = CreateBlock("jscad", {
  defaultExtension: "js",
   ...
   *
   * @param code - Source code from the block (formerly block.value)
   * @param ctx - Block context with blockId, language, params, orgFilePath, blockIndex
   */
  transform: (code, ctx) => {
    // Check for mode from :use parameter (e.g., :use jscad | sourceOnly)
    const useValue = ctx.params.use || 'jscad';
    ...
    const modelModuleId = ctx.params.name
      ? `virtual:org-press:block:jscad-model:${ctx.orgFilePath}:NAME:${ctx.params.name}`
      : `virtual:org-press:block:jscad-model:${ctx.orgFilePath}:${ctx.blockIndex}`;

    // Get height from block parameters or use default
    const height = ctx.params.height || "400px";

    // Use the blockId from context for stable identification
    const stableId = ctx.blockId;
