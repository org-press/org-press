...
/**
 * Org-Press Excalidraw Block Plugin (v2)
 *
 * Provides interactive diagram support for org-mode using Excalidraw
 */

import type { ApiRequest, ApiResponse } from "org-press";
import {
  CreateBlock,
  createBlockId,
...
/**
 * Excalidraw block plugin for org-press (v2 API)
 *
 ...
 */
export const excalidrawPlugin = CreateBlock("excalidraw", {
  defaultExtension: "js",

  transform: (code, ctx) => {
    registerSaveApi();

    const stableId = createBlockId(ctx.orgFilePath, ctx.blockIndex);
    const params = parseBlockParameters(ctx.params.use ? `:use ${ctx.params.use}` : "");

    // Merge parsed params with ctx.params (ctx.params takes priority)
    const mergedParams = { ...params, ...ctx.params };

    const useValue = mergedParams.use || 'excalidraw';
    ...
    const height = mergedParams.height || '500px';
    const blockId = mergedParams.name || stableId;

    const isJsMode = isJavaScriptBlock(code, ctx.language);

    if (isJsMode) {
      const dataModuleId = mergedParams.name
        ? `virtual:org-press:block:excalidraw-data:${ctx.orgFilePath}:NAME:${mergedParams.name}`
        : `virtual:org-press:block:excalidraw-data:${ctx.orgFilePath}:${ctx.blockIndex}`;
