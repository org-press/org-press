 ...
 */
export const echartsPlugin = CreateBlock("echarts", {
  defaultExtension: "js",
   ...
   */
  transform: (code, ctx) => {
    // Parameters are already parsed in ctx.params
    ...
    if (isJsMode) {
      ...
      const dataModuleId = params.name
        ? `virtual:org-press:block:echarts-data:${ctx.orgFilePath}:NAME:${params.name}`
        : `virtual:org-press:block:echarts-data:${ctx.orgFilePath}:${ctx.blockIndex}`;
...
#+begin_src json :use echarts | withTabs
{
...
#+begin_src json :use echarts | withTabs :height 500px
{
...
#+begin_src json :use echarts | withTabs :theme dark
{
