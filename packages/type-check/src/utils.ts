/**
 * Utilities for @org-press/type-check
 *
 * Block collection and config loading functions.
 */
...
import type { CollectedBlock, CollectOptions, TsConfig } from "./types.js";
...
export async function collectCodeBlocks(
  ...
  for (const orgFile of orgFiles) {
    try {
      ...
      console.warn(
        `[type-check] Warning: Failed to parse ${orgFile}: ${
...
export async function loadTsConfig(projectRoot: string): Promise<TsConfig> {
  ...
  try {
    ...
    console.warn(
      `[type-check] Warning: Failed to load tsconfig.json: ${
        error instanceof Error ? error.message : String(error)
      }`
    );
    return {
