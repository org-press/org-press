...
#+DESCRIPTION: AI assistant chat demo

* AI Chat Section

Section demonstrating AI integration with a simulated chat conversation.
...
import { sleep, typewrite } from '../three/animation.org?name=animation';
...
const CHAT_SCRIPT: ChatMessage[] = [
  {
    type: 'error',
    content: "Block renders nothing - output is empty",
    delay: 0,
  },
  {
    type: 'ai',
    content: "Your block is missing `export function render()`. With `:use dom`, you need to export a render function that returns the rendered output.",
    delay: 800,
  },
  {
    type: 'user',
  ...
  {
    type: 'success',
    content: "Added `export function render() { return <Card>{data}</Card>; }` - now rendering!",
    delay: 2200,
  },
];
...
export function createAiChat(): HTMLElement {
  ...
  section.className = 'landing-section ai-chat-section';
  section.id = 'ai';
