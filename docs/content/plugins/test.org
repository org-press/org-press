...
- 🧪 *Literate Testing* - Write tests in the same document as your code
- ⚡ *Vitest Powered* - Fast, modern test runner with excellent DX
- 📊 *Inline Results* - Test results render directly in your documents
- 🔍 *Coverage Support* - Optional code coverage reporting
- 👀 *Watch Mode* - Re-run tests automatically on changes
- 🎯 *Filtering* - Run tests by file or block name
...
#+begin_export html
<pre><code>// In math.org - define the function
#+NAME: add-function
#+begin_src typescript :use sourceOnly
...
├── content/
│   ├── index.org        # Main content
│   ├── math.org         # Math functions + tests
│   └── utils.org        # Utility functions + tests
└── package.json
</code></pre>
#+end_export
...
- [[file:/guide/using-plugins.html][Block Plugin System]] - How plugins work
- [[file:/examples/block-imports.html][Block Imports Guide]] - Importing code between blocks
- [[https://vitest.dev/][Vitest Documentation]] - Full Vitest API reference
- [[file:/plugins/jscad.html][JSCad Plugin]] - 3D modeling plugin
- [[file:/plugins/excalidraw.html][Excalidraw Plugin]] - Diagram plugin
