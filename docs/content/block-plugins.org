...
| Feature | Basic Plugin | Full Plugin (with Preview) |
|-|-|-|
...
** 3. Preview Function (Optional)

Custom preview rendering for the result:
...
Plugins can export a =Preview= function to customize result rendering:

#+begin_src typescript :use sourceOnly
// In the transformed code
export const Preview = (result, ctx) => {
  return {
    html: `<div class="custom-preview">${JSON.stringify(result)}</div>`,
    css: `.custom-preview { background: #f0f0f0; padding: 1rem; }`
  };
};
#+end_src
...
- [[file:api/preview-api.org][Preview API]] - How preview modes work
